...
export async function GET(req: NextRequest, { params }: { params: { token: string } }) {
  ...
  if (link.type === 'photo') {
    const photo = db.prepare('SELECT id, caption, filename, created_at FROM photos WHERE id = ?').get(link.target_id) as any
    ...
    return NextResponse.json({
      ...
      type: 'photo',
      photos: [{ id: photo.id, caption: photo.caption, url: `/api/file/${photo.id}?share=${params.token}` }],
    })
  } else {
    ...
    const photos = db.prepare('SELECT id, caption FROM photos WHERE album_id = ? ORDER BY created_at DESC').all(link.target_id) as any[]
    db.prepare('UPDATE share_links SET view_count = view_count + 1 WHERE id = ?').run(link.id)
    ...
    return NextResponse.json({
      ...
      photos: photos.map(p => ({ id: p.id, caption: p.caption, url: `/api/file/${p.id}?share=${params.token}` })),
      viewerAccess: access,
    })
  }
}
